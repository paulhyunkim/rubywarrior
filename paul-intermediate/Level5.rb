class Player
  	...
  	def play_turn(warrior)
	    ...
	    if !warrior.listen.empty?
	    	if all_direction.any? {|direction| warrior.feel(direction).stairs?}
	    		warrior.walk!(all_direction.find { |direction| !warrior.feel(direction).stairs?})
	    	
			elsif all_direction.select { |direction| warrior.feel(direction).enemy?}.count > 1
				warrior.bind!(all_direction.find { |direction| warrior.feel(direction).enemy?})
		    ...
		    elsif all_direction.any? {|direction| warrior.feel(direction).captive?}
